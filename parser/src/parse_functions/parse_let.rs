...
pub fn parse_let(parser: &mut Parser) -> ParseResult<Statement> {
    let token = parser.cur_token.clone(); // token 'let'

    parser.next_token(); // 离开 'let'
    ...
    Ok(Statement::Let { token, name, var_type, value })
}
